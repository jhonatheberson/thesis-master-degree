function nova_geracao=crossover(populacao_atual)
...
nova_geracao=pais(:,1:end-1);
%|Realizar os cruzamentos|

for i=1:size(pais,1)-1
    for j=2:size(pais,1)
        if randi(2*size(pais,1)-1,1)>i+j
        nova_geracao=[nova_geracao; pais(i,1:size(pais,2)-1)+rand*(pais(i,1:size(pais,2)-1)-pais(j,1:size(pais,2)-1))];
            if rand<0.11
